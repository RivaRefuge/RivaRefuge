...
function nonprofit_wp_title( $title, $sep ) {
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() )
		$title = "$title $sep " . sprintf( __( 'Page %s', 'non-profit' ), max( $paged, $page ) );
	return $title;
}
/**
...
function nonprofit_page_title() {
	...
	if ( ! is_front_page() ) {
		...
		if ( is_category() ) {
			printf( __( 'Category Archives: %s', 'non-profit' ), single_cat_title( '', false ) );
			}
		elseif ( is_tag() ) {
			printf( __( 'Tag Archives: %s', 'non-profit' ), single_tag_title( '', false ) ); 
		}
		elseif ( is_search() ) {
			printf( __( 'Search Results for:', 'non-profit' ) . '&nbsp;' . get_search_query() );
		}
		elseif ( is_archive() ) {
			if ( is_author() ) {
				printf( __( 'All posts by %s', 'non-profit' ), get_the_author() );
			}
			else {
				if ( is_day() ) 
					printf( __( 'Daily Archives: %s', 'non-profit' ), get_the_date() );
				elseif ( is_month() ) 
					printf( __( 'Monthly Archives: %s', 'non-profit' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'non-profit' ) ) );
				elseif ( is_year() ) 
					printf( __( 'Yearly Archives: %s', 'non-profit' ), get_the_date( _x( 'Y', 'yearly archives date format', 'non-profit' ) ) );
				else 
					_e( 'Archives', 'non-profit' );	
			}
		}
		elseif ( is_404() ) {
			_e( 'Page not found', 'non-profit' );
		...
		elseif( is_singular( 'portfolio' )) { 
			_e( 'Portfolio', 'non-profit' ) ;	
		}
		elseif( is_singular( 'gallery' )) {
			_e( 'Gallery', 'non-profit' ) ;	
		}
		elseif( is_page() ) {
			echo the_title() ;
		}	
	...
	switch ( $comment ->comment_type ) :
		...
		// display trackbacks differently than normal comments. ?>
			<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
				<p><?php _e( 'Pingback:', 'non-profit' ); comment_author_link();  edit_comment_link( __( 'Edit', 'non-profit' ), '<span class="edit-link">', '</span>' ); ?></p>
			<?php break;
		default : ?>
		<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
			<div id="comment-<?php comment_ID(); ?>">
				<div class="comment-author vcard">
					...
					<div class="comment-meta commentmetadata"><a href="<?php echo esc_url( htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ) ?>"><?php echo get_comment_date() . '&nbsp;' . __( 'at',  'non-profit' ) . '&nbsp;' . get_comment_time() ; ?></a><?php edit_comment_link( __( 'Edit', 'non-profit' ) ) ?></div>
